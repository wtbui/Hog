...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    maxScore = averaged_roll(1, dice)
    maxRoll = 1

    for i in range(2, 11):
        currScore = averaged_roll(i, dice)
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    score_tail = tail_points(opponent_score)

    if (score_tail > threshold):
        return 0

    if (perfect_square(score + score_tail)):
        if ((next_perfect_square(score) - score) + score_tail > threshold):
            return 0
...
def run(*args):
    ...
    if args.run_experiments:
        ...
        dist move - obstacles
        mxm matrix 2x2 sub matrices, 0 1s 
